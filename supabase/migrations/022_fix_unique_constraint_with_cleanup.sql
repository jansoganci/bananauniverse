-- Migration 022: Fix unique constraint with data cleanup
-- Cleans up duplicate records and creates proper unique index

-- 1. Clean up duplicate records by keeping only the latest one per user/device/date
WITH ranked_quotas AS (
    SELECT 
        ...
        updated_at,
        ROW_NUMBER() OVER (
            PARTITION BY COALESCE(user_id::text, ''), device_id, date 
...
quota_totals AS (
    SELECT 
        COALESCE(user_id::text, '') as user_key,
        device_id,
        date,
        SUM(used) as total_used,
        MAX(limit_value) as max_limit
    FROM daily_quotas
    GROUP BY COALESCE(user_id::text, ''), device_id, date
)
DELETE FROM daily_quotas 
WHERE id IN (
    SELECT dq.id 
    FROM daily_quotas dq
    JOIN ranked_quotas rq ON dq.id = rq.id
    WHERE rq.rn > 1
);

-- 2. Update remaining records with correct used count
...
SET 
    used = qt.total_used,
    limit_value = qt.max_limit,
    updated_at = NOW()
FROM quota_totals qt
WHERE COALESCE(daily_quotas.user_id::text, '') = qt.user_key
AND daily_quotas.device_id = qt.device_id
AND daily_quotas.date = qt.date;
...
-- 4. Create a COALESCE-based unique index that treats NULL user_id as empty string
CREATE UNIQUE INDEX daily_quotas_unique_user_device_date 
ON daily_quotas (COALESCE(user_id::text, ''), device_id, date);

-- 5. Update the consume_quota function to use the new unique constraint logic
