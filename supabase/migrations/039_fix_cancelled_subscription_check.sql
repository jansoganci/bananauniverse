-- =====================================================
-- Migration 039: Fix Cancelled Subscription Handling
-- Purpose: Exclude cancelled subscriptions from premium check
-- Security: Prevent cancelled users from accessing premium features
-- =====================================================

-- =====================================================
-- Fix consume_quota Premium Check
-- =====================================================
-- ISSUE: Previously checked status='active' AND expires_at > NOW()
-- Missing: Explicit exclusion of 'cancelled' status
-- FIX: Add defense-in-depth check for cancelled subscriptions
...
BEGIN
    ...
    SELECT EXISTS(
        ...
        AND status != 'cancelled'  -- ✅ NEW: Defense in depth - exclude cancelled
    ) INTO v_is_premium;
...
-- Migration Complete
-- =====================================================
-- ✅ FIX #1 COMPLETE: consume_quota() now excludes cancelled subscriptions
-- Security: Cancelled users can no longer access premium features
-- Testing: Re-run Test 2.5 to verify cancelled subscription handling
