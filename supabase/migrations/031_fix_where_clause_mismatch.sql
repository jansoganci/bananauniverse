-- Migration 031: Fix WHERE Clause Mismatch in consume_quota Function
-- The UPDATE WHERE clause doesn't match the unique index structure
...
BEGIN
    ...
    -- UPSERT with corrected WHERE clause to match unique index
    BEGIN
    ...
    EXCEPTION
        WHEN unique_violation THEN
            ...
            -- FIXED: WHERE clause now matches the unique index structure with NULL-safe comparison
            -- Unique index: (COALESCE(user_id::text, ''), device_id, date)
            -- WHERE clause: COALESCE(user_id::text, '') = COALESCE(p_user_id::text, '') AND device_id IS NOT DISTINCT FROM p_device_id
            UPDATE daily_quotas
            SET 
                used = used + 1,
                updated_at = NOW()
            WHERE COALESCE(user_id::text, '') = COALESCE(p_user_id::text, '')
            AND device_id IS NOT DISTINCT FROM p_device_id  -- FIXED: NULL-safe comparison
            AND date = v_today
            RETURNING used, limit_value, id INTO v_used, v_limit, v_record_id;
...
-- Comments
COMMENT ON FUNCTION consume_quota(UUID, TEXT, BOOLEAN, UUID) IS 'Fixed WHERE clause to match unique index structure with NULL-safe comparison (IS NOT DISTINCT FROM) for proper UPDATE operations';
