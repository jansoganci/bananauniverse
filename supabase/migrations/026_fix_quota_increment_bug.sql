-- Migration 026: Fix quota increment bug in consume_quota function
-- 
-- ROOT CAUSE ANALYSIS:
-- 1. Unique index uses COALESCE for both user_id and device_id: 
--    daily_quotas_unique_user_device_date ON (COALESCE(user_id::text, ''), device_id, date)
-- 2. UPDATE query doesn't handle NULL device_id correctly: WHERE device_id = p_device_id
--    This fails when p_device_id is NULL because NULL = NULL is always FALSE in SQL
-- 3. RLS policies block UPDATE operations for anonymous users because request.device_id 
--    session variable is not set
--
-- SOLUTION:
-- 1. Set session variable at start of function for anonymous users
-- 2. Fix WHERE clause to match unique index structure exactly
-- 3. Ensure proper NULL handling in all queries
...
VALUES ('quota_increment_bug_fix', 
        jsonb_build_object(
            'migration', '026_fix_quota_increment_bug',
            'fixes', ARRAY[
                'Set request.device_id session variable for RLS policies',
                'Fix WHERE clause to match unique index structure',
                'Proper NULL handling in all queries',
                'Enhanced logging for debugging'
            ],
            'expected_impact', 'Quota increment should work correctly for all user types'
        ), 
        NOW());
...
DO $$
BEGIN
    RAISE LOG '[QUOTA-FIX] ✅ Migration 026: Quota increment bug fix applied successfully';
    RAISE LOG '[QUOTA-FIX] Key fixes:';
    RAISE LOG '[QUOTA-FIX] - Session variable set for RLS policies';
    RAISE LOG '[QUOTA-FIX] - WHERE clause matches unique index structure';
    RAISE LOG '[QUOTA-FIX] - Proper NULL handling implemented';
END $$;
